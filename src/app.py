...
def create_metrics(write_csv: bool = False) -> pd.DataFrame:
    ...
    df['risk_cryptoverse'] = normalise(np.log(df['sma_50d'] / df['sma_50w'] * df['poly']))
    df['risk_diff'] = normalise(df['log_close'] - df['poly'])
    df['risk_logpoly'] = normalise(np.log(df['risk_diff']+1)*df['poly']).rolling(10).mean()
...
def create_figures(df: pd.DataFrame) -> list[tuple]:
    ...
    figures.append(
        ('risk_metrics', fig, 'Timeseries of risk metrics with the buy/sell ranges overlayed')
    )
    ...
    fig = update_margin(fig)
    figures.append(
        ('price_ts', fig, 'Timeseries of close price, 50 day and 50 week moving averages')
    )
...
def send_email(message: MIMEMultipart) -> None:
    ...
    if (email_address is None) or (password is None):
        raise ValueError('GMAIL and PASSWORD environment variables and needed to send email.')
    try:
